...
function parse(formData: FormData) {
  const value = {
    ...
    accountId: String(formData.get("account_id") ?? ""), dayOfMonth: Number(formData.get("day_of_month")),
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(value.startDate) || (value.endDate && (!/^\d{4}-\d{2}-\d{2}$/.test(value.endDate) || value.endDate < value.startDate))) return { error: "Informe um período válido." } as const;
  if (!value.accountId) return { error: "Selecione uma conta." } as const;
  return { value } as const;
}

async function validateRelations(workspaceId: string, accountId: string, categoryId: string | null) {
...
export async function createRecurrence(_state: ActionState, formData: FormData): Promise<ActionState> {
  ...
  try {
    ...
    const { supabase, user } = await validateRelations(workspace.id, parsed.value.accountId, parsed.value.categoryId);
    const { error } = await supabase.from("recurrence_rules").insert({ workspace_id: workspace.id, created_by: user.id, name: parsed.value.name, transaction_type: parsed.value.transactionType, amount: parsed.value.amount, amount_type: parsed.value.amountType, frequency: "monthly", category_id: parsed.value.categoryId, account_id: parsed.value.accountId, day_of_month: parsed.value.dayOfMonth, start_date: parsed.value.startDate, end_date: parsed.value.endDate });
    if (error) return { error: `Não foi possível criar a recorrência: ${error.message}` };
...
export async function updateRecurrence(_state: ActionState, formData: FormData): Promise<ActionState> {
  ...
  try {
    const workspace = await getActiveWorkspace(); if (!workspace) return { error: "Nenhum workspace disponível." };
    const { supabase } = await validateRelations(workspace.id, parsed.value.accountId, parsed.value.categoryId);
    ...
    const { error } = await supabase.from("recurrence_rules").update({ name: parsed.value.name, transaction_type: parsed.value.transactionType, amount: parsed.value.amount, amount_type: parsed.value.amountType, category_id: parsed.value.categoryId, account_id: parsed.value.accountId, day_of_month: parsed.value.dayOfMonth, start_date: parsed.value.startDate, end_date: parsed.value.endDate }).eq("id", id).eq("workspace_id", workspace.id);
    if (error) return { error: `Não foi possível atualizar a recorrência: ${error.message}` };
    await ensureRecurrenceWindow(workspace.id);
  } catch (error) { return { error: error instanceof Error ? error.message : "Erro inesperado." }; }
...
export async function deactivateRecurrence(_state: ActionState, formData: FormData): Promise<ActionState> {
  ...
  try { const workspace = await getActiveWorkspace(); if (!workspace) return { error: "Nenhum workspace disponível." }; const { supabase } = await requireWorkspaceMembership(workspace.id); const { error } = await supabase.from("recurrence_rules").update({ active: false }).eq("id", id).eq("workspace_id", workspace.id).eq("active", true); if (error) return { error: `Não foi possível desativar: ${error.message}` }; }
  catch (error) { return { error: error instanceof Error ? error.message : "Erro inesperado." }; }
